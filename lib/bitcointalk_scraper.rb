...
module BitcointalkScraper
	...
	def self.scrape_all
		...
		(20..40).step(20).each do |num|	
			...
			# set up fuzzy string matching
			m = JaroWinkler.new("TWITTER REPORT")

			# get the parent tag for posts that contain "TWITTER REPORT" using fuzzy matching
			#posts = page.search('div.post').select { |post| post.inner_html.include?("TWITTER REPORT") }.map { |p| p.parent.parent }
			posts = page.search('div.post').select { |post| m.match(post.inner_html) }.map { |p| p.parent.parent }
